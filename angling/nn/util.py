...
def _get_combination(combination: str, tensors: List[torch.Tensor]) -> torch.Tensor:
    if combination.isdigit():
        return tensors[int(combination)]
    else:
...
def get_combined_dim(combination: str, tensor_dims: List[int]) -> int:
    ...
    combination = combination.replace("x", "0").replace("y", "1")
    return sum(_get_combination_dim(piece, tensor_dims) for piece in combination.split(","))


def _get_combination_dim(combination: str, tensor_dims: List[int]) -> int:
    if combination.isdigit():
        return tensor_dims[int(combination)]
    else:
        if len(combination) != 3:
            raise Exception("Invalid combination: " + combination)
...
def _get_combination_and_multiply(
    ...
    if combination.isdigit():
        return torch.matmul(tensors[int(combination)], weight)
    else:
        if len(combination) != 3:
            raise Exception("Invalid combination: " + combination)
        first_tensor = _get_combination(combination[0], tensors)
        ...
        if operation == "*" or operation == "/":
            ...
            if first_tensor.dim() == 4:
                # [batch, 1, sequence_len, dim] 4 dim
            ...
            return torch.matmul(first_tensor + second_tensor, weight)
        elif operation == "-":
            return torch.matmul(first_tensor - second_tensor, weight)
        else:
            raise Exception("Invalid operation: " + operation)
